...
function mergeObjects(objects) {
  ...
  objects.forEach((object) => {
    Object.entries(object).forEach(([key, value]) => {
      temp[key] = temp[key] ? temp[key] + value : value;
    });
  });
  return temp;
}
...
function makeWord(lettersObject) {
  ...
  const elems = Object.values(lettersObject);
...
function fromJSON(proto, json) {
  ...
  Object.assign(temp, JSON.parse(json));
  return temp;
}
...
function sortCitiesArray(arr) {
  return arr.sort((a, b) => {
    const temp = a.country.localeCompare(b.country);
    if (temp === 0) {
...
function group(array, keySelector, valueSelector) {
  const temp = new Map();
  array.forEach((elem) => {
    if (temp.has(keySelector(elem))) {
      temp.set(keySelector(elem), [
        ...temp.get(keySelector(elem)),
        valueSelector(elem),
      ]);
    } else {
      temp.set(keySelector(elem), [valueSelector(elem)]);
    }
  });
  return temp;
}
